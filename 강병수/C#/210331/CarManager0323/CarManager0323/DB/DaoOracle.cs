...
namespace CarManager0323.DB
{
    class DaoOracle
    {
        ...
        public void insertTrade(Customer cust, Car car, Seller seller)
        {
            try
            {
                string sql = string.Format("insert into trade values (trade_SEQ.nextval, sysdate, " + 
                            "(select cr_id from seller where cr_name = '{0}' and " +
                            "cr_tel = '{1}'), " +
                            "(select car_id from car where model = '{2}' and " +
                            "price = '{3}'), " +
                            "(select cus_id from customer where cus_name = '{4}' and " +
                            "cus_tel = '{5}'))", seller.Name, seller.Tel , car.Model, car.Price , cust.Name, cust.Tel);
                /*" (select s.cr_id from seller s, customer cu where cu.cus_id = s.cr_id), " +
                " (select cu.cus_id from customer cu, car ca where ca.car_id = cu.cus_ID)," +
                " (select ca.car_id from car ca, customer cu where cu.cus_id = ca.car_id))";*/
                cmd.Connection = conn;
        ...
        public List<DealResult> selectDeal()
        {
            ...
            {
                int count = 1;
                string query = "select cus_name, model, price, cr_name, tr_date " +
                               "from trade d, car ca, customer cu, seller se " +
                               "where d.car_id = ca.car_id and d.cus_id = cu.cus_id " +
                               "and d.cr_id = se.cr_id";
                cmd.Connection = conn;
                cmd.CommandText = query;
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        Console.WriteLine("고객명: " + reader["cus_name"]);
                        Console.WriteLine("모델: " + reader["model"]);
                        Console.WriteLine("가격: " + reader["price"]);
                        Console.WriteLine("판매자: " + reader["cr_name"]);
                        Console.WriteLine("구매날짜: " + reader["tr_date"]);
                        Console.WriteLine("-----------------------------");
                        DealResult dResult = new DealResult(count, reader["cus_name"].ToString(), reader["model"].ToString(),
                            reader["price"].ToString(), reader["cr_name"].ToString(), reader["tr_date"].ToString());
